...
const PID: ProviderId = "mistral";
const BASE = getProviderUrl(PID);

function authHeaders(): Record<string, string> {
  const key = getApiKey(PID);
  if (!key) throw new Error("Mistral API key not set. Add it in Settings.");
  return {
...
export async function embedMistral(texts: string[]): Promise<number[][]> {
  const headers = authHeaders();
