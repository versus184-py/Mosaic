...
function MessageNode({ id, data: rawData }: NodeProps) {
  ...
  const isError = data.label === "⚠ Error — click to retry";
  ...
  return (
    <motion.div
      ...
      style={{ cursor: "pointer", width: nodeWidth, position: "relative", borderRadius: 20 }}
    >
      {childCount > 0 && (
        <motion.button
              ...
              {msg.role === "assistant" && (
                <div style={{
                  ...
                  <span style={{
                    ...
                    background: "linear-gradient(135deg, var(--accent), rgba(100,180,255,0.5))",
...
function areEqual(prev: NodeProps, next: NodeProps) {
  ...
  if (prevData.label !== nextData.label || prevData.isActive !== nextData.isActive ||
      ...
      prevData.nodeType !== nextData.nodeType) return false;
